...
\begin{gather}
\zeta = arg\max \zeta^{*}W_cW_o\zeta s.t. ||\zeta||_2^{2} = 1 \\
...
As shown here \cite{•} the remaining derivative can be solved in the following way:
\begin{gather}
\frac{d}{dx} x^{*}Ax = 2Ax \label{der-mat}
\end{gather}
This holds if \(A\) is symmetric. 
Here both \(W_c\) and \(W_o\) share the same set of eigenvectors \ref{eigendec}, therefore they commute \cite{170371}.
